...
# Here, the package "reshape" is needed
PP_cast <- cast(aggregate(Gdata_PP$value,
...
# Changing the species to front column and deleting the species column
rownames(PP_cast) <- PP_cast$Group.1
PP_cast <- PP_cast[,-1]
```
...
# Save results as tables
write.table(EBN_2014_P, file = "Temporal_niche_overlap_probabilities_EBN_2014_Pianka.txt", sep = "\t", row.names = F, col.names = c("Species 1", "Species 2", "Temporal Niche Overlap Probability"))
write.table(EBN_2014_C, file = "Temporal_niche_overlap_probabilities_EBN_2014_Czekanowski.txt", sep = "\t", row.names = F, col.names = c("Species 1", "Species 2", "Temporal Niche Overlap Probability"))
```
...
Overview graph with all Pianka and Czekanowski temporal niche probabilites for EBN 2007-2015 and Peru (Panguana) 2010, 2013 and 2015:
...
par(mfrow=c(4,3))
  ...
  plot(PP_2015_P[,3], PP_2015_C[,3],
     ...
     ylab = "Czekanowski",
     pch = 3)
#  mtext("Niche Overlap Probabilities, each cross = one pair of spp")
```
