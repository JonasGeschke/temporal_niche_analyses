---
title: "__Statistical analysis of co-occurrence interactions within tropical moth__"
...
setwd("~/Studium/MSc/3. Semester/Co-occurrence/GitHub/co-occurence")
...
Gdata <- droplevels(Gdata)
summary(Gdata$light)
### There is some "m" within the values - what to do with it?
...
Also, for later co-occurrence analyses, we create a "value" column.
...
```{r, include=FALSE}
########################## brauchen wir nicht mehr, glaube ich ###########################
# Gdata <- cbind(Gdata, "date_r" = as.Date(Gdata$date, "%d.%m.%Y"))
# Gdata <- cbind(Gdata, "date_num" = as.numeric(Gdata$date))
# Gdata <- cbind(Gdata, "monthday" = format(Gdata$date_r, "%m-%d"))
# Gdata <- cbind(Gdata, "day_nr" = yday(Gdata$date_r))
# Gdata <- cbind(Gdata, "month" = format(Gdata$date_r, "%m"))
# Gdata <- cbind(Gdata, "Month" = format(Gdata$date_r, "month.names"))
# str(Gdata)
##########################################################################################
```
...
plot0 <- ggplot(Gdata, aes(x = day_nr, fill = monat))+
...
As we can see in that plot, is does not make sense to include those data points, where we neither know the trapping site nor the trapping year. So next, we delete those rows out of the datatable.

```{r, results='hide'}
# Creating a new "Gdata1"
Gdata1 <- Gdata

# Deleting all rows, where no "site" is specified
summary(Gdata1$site)
Gdata1 <- subset(Gdata1, site != "NA")
Gdata1 <- droplevels(Gdata1)
summary(Gdata1$site)

# Deleting all rows, where no "year" is specified.
# This means that we actually have to delete the "date" column,
# as we previously got the year information from here.
summary(Gdata1$date)
Gdata1 <- subset(Gdata1, date != "NA")
Gdata1 <- droplevels(Gdata1)
summary(Gdata1$date)

# Controlling that there are no more NAs in the "date" and "year" columns
summary(Gdata1$year)
# As there still is one NA within the "year" column, we also delete this
summary(Gdata1$year)
Gdata1 <- subset(Gdata1, year != "NA")
Gdata1 <- droplevels(Gdata1)
# And we controll this process again
summary(Gdata1$year)
# Done!
```

Now one could repeat plotting this.

```{r, fig.show='hide'}
# Creating plot1. This plot shows the data without site and year NAs!
plot1 <- ggplot(Gdata1, aes(x = day_nr, fill = monat))+
  scale_fill_manual(name = "Month", values = colourpalette10,
                    breaks = c("Januar",
                               "Februar",
                               "März",
  ...
  facet_wrap(site ~ year, ncol = 1)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  labs(title = "Number of trapped individuals per site",
       x = "Day of the year",
       y = "No of trapped Arctiinae individuals")+
  scale_x_continuous(expand = c(0,0),
                     breaks = seq(0, 365, 7),
...
Next, we would like to have such a plot just with the data about Arctiinae and only those datapoint with 2nd trust level. So we delete all other datapoints. Also, it does not make sense to keep rows where we do not know the species number.

```{r, results='hide'}
# Creating a new "Gdata2"
Gdata2 <- Gdata1

# Deleting all rows, in where it is no "Arct" in the "taxon" column
Gdata2 <- subset(Gdata2, taxon == "Arct")
Gdata2 <- droplevels(Gdata2)
summary(Gdata2$taxon)

# Deleting all rows, in where it is no trust level "2" in the "trust" column
Gdata2 <- subset(Gdata2, trust == "2")
Gdata2 <- droplevels(Gdata2)
summary(Gdata2$trust)

# Deleting all rows, where no "spp" is identified
Gdata2 <- subset(Gdata2, spp != "NA")
Gdata2 <- droplevels(Gdata2)
summary(Gdata2$spp)
```

Now, we repeat plotting this. This plot includes all the datapoints that probably are more interesting to look at, as all basic information are given.

```{r, fig.cap='plot2', fig.ext='pdf', fig.height=27, fig.width=17, dpi=300}
# Creating plot2.
plot2 <- ggplot(Gdata2, aes(x = day_nr, fill = monat))+
  scale_fill_manual(name = "Month", values = colourpalette10,
                    breaks = c("Januar",
                               "Februar",
                               "März",
