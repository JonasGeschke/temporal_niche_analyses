...
3. Run of observation datasets
    => RA4_nicheoverlap function, ra4 = FALSE, input: results of 2
...
5. Run of randomized datasets, only with 0.75prob species
    => RA4_nicheoverlap function, ra4 = TRUE, input: results of 4
...
cast_EBN <- cast(Gdata, site = "EBN")
cast_PP <- cast(Gdata, site = "PP")
```
...
As function:
```{r}
# get the species of the species pairs that show >= 0.75 overlap in the observations datasets
cast_site_75 <- function(data, site = "missing"){
  if(site %in% c("PP", "EBN")){
    #extracting the species from spp pairs prob >=0.75 into single datasets
    sub_data_1 <- data$Spp1[data$TempOverlapProb>=0.75]
    sub_data_2 <- data$Spp2[data$TempOverlapProb>=0.75]
    #create one dataset with all the species just extracted
    if(site == "PP")
    {cast_75 <- cast_PP[rownames(cast_PP) %in% sub_data_1 | rownames(cast_PP) %in% sub_data_2,]
      return(cast_75)}
    else
    {cast_75 <- cast_EBN[rownames(cast_EBN) %in% sub_data_1 | rownames(cast_EBN) %in% sub_data_2,]
      return(cast_75)}}
  else
      {return("Error: Please set site to EBN or PP")}
}
...
cast_EBN_75 <- cast_site(DATA, site = "EBN")
cast_PP_75 <- cast_site(DATA, site = "PP")
```
...
#write_txt function
organized_txt <- function(data_spp){
