...
setwd("C:/RFiles/temporal niche analysis")
...
tail_probability <- function(observed_overlap, random_overlap){
        ...
        for(i in 1:nrow(data)){
                lower_dens <- density(data[i,4:ncol(data)],
                                      to = data[i, 3],
                                      n = 1000000)
                upper_dens <- density(data[i,4:ncol(data)],
                                      from = data[i, 3],
                                      n = 1000000)
                ...
                difference <- ifelse(lower_tail_prob > upper_tail_prob,
                                     "lower", "higher")
                results <- rbind(results, c(data[i, 1:3],
                                            difference,
                                            p_value))
        }
        return(results)
...
dens
lines( dens2 <- density(x, from = 1, n = 100000),
       col="red", lwd = 2)
lines( dens3 <- density(x, to = 1, n = 100000),
       col="blue", lwd = 2)
str(dens2)
...
# As a function, the code block needs to be loaded once for running afterwards
>>>>>>> bb5b83a53ae1751f21e37cb7d0960839297fe9d7
...
# As a function, the code block needs to be loaded once for running afterwards
nicheoverlap <- function(data, method = "missing", ra4 = TRUE, iterations = 1, txt = FALSE, site = "missing", subset = FALSE){
  if(site %in% c("PP", "EBN")){
