...
3. Run of observation datasets
    => RA4_nicheoverlap function (!!! HIER MÜSSEN WIR DEN RA4 noch einmal umgehen !!!)
...
cast_site <- function(data, site = "missing"){
  if (site[1] == "PP"){
    ...
    data_PP <- subset(data, site == "PP")
    #transforming the data into cast-format
    cast_PP <- cast(aggregate(data_PP$value,
                       by = list(data_PP$spp, data_PP$date),
                       FUN = "sum",
                       na.rm = T),
                  Group.1 ~ Group.2,
                  value = "x",
    ...
    rownames(cast_PP) <- cast_PP$Group.1
    cast_PP <- cast_PP[,-1]
    #return cast_site #### DOES NOT WORK YET
    return(data.frame(cast_PP))
    } else
      if (site[1] == "EBN"){
        #subset, which only contains data from EBN
        data_EBN <- subset(data, site == "EBN")
        #transforming the data into cast-format
        cast_EBN <- cast(aggregate(data_EBN$value,
                                  by = list(data_EBN$spp, data_EBN$date),
                                  FUN = "sum",
                                  na.rm = T),
                        Group.1 ~ Group.2,
                        value = "x",
...
#ra4 randomization
RA4_nicheoverlap <- function(data, method = "missing", iterations = 1, site = "missing"){
  if(site %in% c("PP", "EBN")){
    ...
    repeat{
      ...
      #ra4 randomization
      data_rand <- ra4_mod(rel_data)
      if(method[1] == "pianka"){
        # Pianka
        data_p <- cbind(t(combn(rownames(data_rand), 2)), 0)
        for (i in 1:nrow(data_p)){
          data_p[i, 3] <- sum(data_rand[data_p[i,1],] * data_rand[data_p[i,2],])/sqrt(sum(data_rand[data_p[i,1],]^2) * sum(data_rand[data_p[i, 2],]^2))
          }} else
      {if(method[1] == "czekanowski"){
        # Czekanowski
        data_p <- cbind(t(combn(rownames(data_rand), 2)), 0)
        for (i in 1:nrow(data_p)){
          data_p[i, 3] <- 1 - 0.5 * sum(abs((data_rand[data_p[i, 1], ] - data_rand[data_p[i, 2], ])))
        }} else
    ...
    colnames(data_spp) <- c("Spp1", "Spp2", (paste("RA4prob", 1:(ncol(data_spp)-2), sep = "_")))
    write.table(data_spp,
                file = paste("~/", method, "/", site, "/", method,"_", site,"_", Sys.Date(), "_",
...
readin <- function(site, method){
  ...
  colnames(data) <- c("Spp1", "Spp2", (paste("RA4prob", 1:(ncol(data)-2), sep = "_")))
    return(data)
  }
