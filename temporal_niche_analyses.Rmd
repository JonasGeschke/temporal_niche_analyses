...
tail_probability <- function(observed_overlap, random_overlap){
        ...
        results <- data.frame(Spp1 = vector(mode = "character"),
                              ...
                              unique_values = vector(mode = "numeric"),
                              randomisation_count = vector(mode = "numeric")
                              stringsAsFactors = F)
        # COMMENT
        for(i in 1:nrow(data)){
          ...
          else {lower_dens <- density(as.numeric(data[i, 4:ncol(data)]),
                                      to = data[i, 3],
                                      n = 1000)
          ...
          results[nrow(results) + 1,] <- c(as.character(data[i, 1]),
                                           ...
                                           length(unique(data[i, 4:ncol(data)])),
                                           (ncol(data)-3))
          }
        }
        return(results)
...
nicheoverlap <- function(data, subset = FALSE, method = "missing", ra4 = TRUE, iterations = 1, txt = FALSE, site = "missing"){
  if(site %in% c("PP", "EBN")){
    ...
    repeat{
      ...
      comb <- t(combn(rownames(rel_data)))
      colnames(comb) <- c("Spp1", "Spp2")
      row_iterator <- ifelse(subset == FALSE, 1:nrow(data_p),
                             which(complete.cases(merge(comb, subset,
