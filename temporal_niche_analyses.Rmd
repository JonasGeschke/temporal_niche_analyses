...
cast_site <- function(data, site = "missing"){
  if (site == "PP"){
    ...
    else {return("Error: Please set site to EBN or PP")}
  ...
  cast <- cast(aggregate(data_sub$value,
    ...
    cast <- cast[,-1]
    if (site == "PP")
    {cast_PP <- data.frame(cast)
    return(cast_PP)}
    else
    {cast_EBN <- data.frame(cast)
      return(cast_EBN)}
}
```
#    ifelse (site == "PP", (cast_PP <- data.frame(cast)), (cast_EBN <- data.frame(cast)))
