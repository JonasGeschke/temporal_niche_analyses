...
ra4_mod <- function(speciesData) 
{
    ...
    data_mult_obs <- speciesData[rowSums(data != 0) > 1,]
    data_solo_obs <- data[rowSums(data != 0) == 1,]
...
RA4_nicheoverlap <- function(data, method = "missing", iterations = 1, site = "missing")
  {if(site %in% c("PP", "EBN")){
    data_spp <- data.frame(Spp1 = factor(rep(1, choose(nrow(cast_PP_75), 2))),
                           Spp2 = factor(rep(1, choose(nrow(cast_PP_75), 2))))
    repeat{
      ...
      data_spp <- data.frame(data_spp, RA4prob = data_p[,3])
