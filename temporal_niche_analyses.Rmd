...
names(Gdata)
colnames(Gdata) <- c("taxon", "notes", "light", "spp", "date", "site", "time", "iid", "sex", "trap", "trust")
names(Gdata)
```
...
```{r}
# This code block is the cast_"site" function,
# which needs to be loaded one time in order to run it afterwards
...
########### HIER MUSS DIE nicheoverlap FUNKTION EINGEBUNDEN WERDEN ###########
...
plot(EBN_p[,3], EBN_c[,3],
     ...
     pch = 3)
  mtext("(Each cross symbolizes one pair of species)")
  #adding the subset datasets (>=0.5 prob) to the plot, subsetting code see below
  abline(h = 0.5, v = 0.5, lty = 3, col = "grey")
  points(EBN_p_cut0.5[,3], EBN_c_cut0.5[,3], pch = 20, col = "red")

#adding the upper probabilities (>=0.5) as red crosses, as they are used for RA4 analyses
#subsetting the data, so only >=0.5 probability species pairs are in the dataset
EBN_p_cut0.5 <- subset(EBN_p, TempOverlapProb >= 0.5)
EBN_c_cut0.5 <- subset(EBN_c, TempOverlapProb >= 0.5)
```
...
plot(PP_p[,3], PP_c[,3],
     ...
     pch = 3)
  mtext("(Each cross symbolizes one pair of species)")
  #adding the subset datasets (>=0.75 prob) to the plot, subsetting code see below
  abline(h = 0.75, v = 0.75, lty = 3, col = "grey")

#adding the upper probabilities (>=0.5) as red crosses, as they are used for RA4 analyses
#subsetting the data, so only >=0.5 probability species pairs are in the dataset
PP_p_cut0.75 <- subset(PP_p, TempOverlapProb >= 0.75)
PP_c_cut0.75 <- subset(PP_c, TempOverlapProb >= 0.75)
```
